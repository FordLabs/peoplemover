...
describe('Share Access Form', () => {
    beforeEach(() => {
        ...
        cy.get('[data-testid=modalCard]').eq(0).as('inviteToViewModalCard');

        cy.get('[data-testid=modalCard]').eq(1).as('inviteToEditModalCard');
    });
    ...
    context('"Invite others to view" Section', () => {
        it("should toggle the space's view-only switch and update the space accordingly", () => {
            cy.get('[data-testid=viewOnlyAccessToggle]').as(
                'toggleReadOnlySwitch'
            );
            cy.get('@toggleReadOnlySwitch')
                .should('not.be.checked')
                .siblings('.react-switch-handle')
                .click();
            cy.get('@toggleReadOnlySwitch').should('be.checked');
        });
    });
    ...
    context('"Invite others to edit" Section', () => {
        ...
        it('Invite editors to a space should show link to space url', () => {
            cy.server();
            cy.route('POST', Cypress.env('API_USERS_PATH')).as(
                'postAddPersonToSpace'
            );
            const spaceUuid = Cypress.env('SPACE_UUID');
            const baseUrl = Cypress.config().baseUrl;

            cy.get('[data-testid=userIdName]')
                .eq(0)
                .should('contain.text', 'USER_ID');

            cy.get('[id=employeeIdTextArea]')
                .focus()
                .type('aaaaaa#', { force: true });
            ...
            cy.get('[data-testid=inviteEditorsFormSubmitButton]')
                .should('not.be.disabled')
                .click();

            cy.wait('@postAddPersonToSpace').should(
                (xhr: Cypress.ObjectLike) => {
            ...
            cy.get('[data-testid=grantEditAccessConfirmationFormDoneButton]')
                .focus()
                .click();
            ...
            cy.get('[data-testid=userListItem__ELISE]')
                ...
                .find(':contains("Editor")')
                .eq(0)
                .click();
            cy.get('[data-testid=userAccessOptionLabel]')
                .eq(2)
        ...
        it('Transferring ownership', () => {
            ...
            cy.route('PUT', `${Cypress.env('API_USERS_PATH')}/ELISE`).as(
                'putChangeOwner'
            );
            cy.route('GET', Cypress.env('API_USERS_PATH')).as('getAllUsers');

            cy.log(
                '**Transferring ownership to an editor should change current owner to editor**'
            );
            ...
            cy.get(
                '[data-testid=grantEditAccessConfirmationFormDoneButton]'
            ).click();
            cy.get('[data-testid=modalPopupContainer]').should('not.exist');
            ...
            cy.wait('@putChangeOwner').should((xhr: Cypress.ObjectLike) => {
                expect(xhr.status).to.equal(200);
            });

            cy.wait('@getAllUsers').should((xhr: Cypress.ObjectLike) => {
                expect(xhr.status).to.equal(200);
            });

            cy.get('[data-testid=userListItem__USER_ID]').should(
                'contain.text',
                'USER_ID'
            );
            cy.get('[data-testid=userListItem__USER_ID]').should(
            ...
            cy.get('[data-testid=userListItem__ELISE]').should(
                'contain.text',
                'ELISE'
            );
            cy.get('[data-testid=userListItem__ELISE]').should(
            ...
            cy.log(
                '**Removing current user from space should direct user to dashboard**'
            );
            cy.get('[data-testid=userListItem__USER_ID]')
                .should('contain.text', 'USER_ID')
                .find(':contains("Editor")')
            ...
            cy.log(
                '**The space current user was removed from should no longer be on dashboard**'
            );
...
const inviteToViewModalCardShouldBeExpanded = (): void => {
    cy.get('@inviteToViewModalCard').should(
        'have.attr',
        'aria-expanded',
        'true'
    );
    cy.get('@inviteToEditModalCard').should(
        'have.attr',
        'aria-expanded',
        'false'
    );
