...
describe('Product', () => {
    ...
    it('Create a new product', () => {
        cy.intercept('POST', Cypress.env('API_PRODUCTS_PATH')).as(
            'postNewProduct'
        );
        cy.intercept(
            'POST',
            Cypress.env('API_LOCATION_PATH'),
            cy.spy().as('locationCall')
        ).as('postNewLocation');
        cy.intercept(
            'POST',
            Cypress.env('API_PRODUCT_TAG_PATH'),
            cy.spy().as('productTagsCall')
        ).as('postNewTag');
        ...
        cy.log(
            '**Existing location tags should be in the locations dropdown**'
        );
        getProductForm()
            .find('[id=location]')
            .focus()
        ...
        getProductForm().contains('location1').should('exist');
        cy.get('@locationCall').its('callCount').should('equal', 0);

        cy.log(
            '**Existing product tags should be in the product tags dropdown**'
        );
        getProductForm()
            .find('[id=productTags]')
            .focus()
        ...
        getProductForm().contains('productTag1').should('exist');
        ...
        cy.wait('@postNewProduct').then((interception) => {
            ...
            expect(body.startDate).to.equal(
                product.startDate.format('yyyy-MM-DD')
            );
            expect(body.endDate).to.equal(
                product.nextPhaseDate.format('yyyy-MM-DD')
            );
    ...
    it('Edit a product', () => {
        ...
        cy.intercept('PUT', Cypress.env('API_PRODUCTS_PATH') + '/**').as(
            'updateProduct'
        );
        ...
        cy.wait('@updateProduct').then((interception) => {
            ...
            expect(body.startDate).to.equal(
                updateProduct.startDate.format('yyyy-MM-DD')
            );
            expect(body.endDate).to.equal(
                updateProduct.nextPhaseDate.format('yyyy-MM-DD')
            );
    ...
    context('Delete a product', () => {
        it('that is active', () => {
            cy.intercept('DELETE', Cypress.env('API_PRODUCTS_PATH') + '/**').as(
                'deleteProduct'
            );
            ...
            cy.get('[data-testid=editProductIcon__baguette_bakery]').should(
                'not.exist'
            );
        });
        ...
        it('that is archived', () => {
            ...
            cy.get('[data-testid=archivedProductsDrawerCountBadge]').should(
                'have.text',
                1
            );
            cy.contains(
                '[data-testid=archivedProductsDrawerCaret]',
                'Archived Products'
            ).click();
            ...
            cy.get('[data-testid=deleteProduct]').contains('Delete').click();
            cy.get('[data-testid=confirmationModalArchive]').should(
                'not.exist'
            );
            cy.get('[data-testid=confirmDeleteButton]').click();
    ...
    context('Product name field warnings', () => {
        ...
        it('Display duplicate product name warning if product name is a duplicate', () => {
            ...
            const expectedDuplicateProductNameWarningMessage =
                'A product with this name already exists. Please enter a different name.';
            cy.get('[data-testid=productNameWarningMessage]').should(
                'have.text',
                expectedDuplicateProductNameWarningMessage
            );
        });
        ...
        it('Display empty product name warning if product name is empty', () => {
            ...
            const expectedEmptyProductNameWarningMessage =
                'Please enter a product name.';
            cy.get('[data-testid=productNameWarningMessage]').should(
                'have.text',
                expectedEmptyProductNameWarningMessage
            );
        });
    });
});
...
const populateProductForm = (
    { name, location, tags = [], startDate, nextPhaseDate, notes },
    defaultStartDate
): void => {
    ...
    cy.get('@calendarStartDate').should('have.value', defaultStartDate).click();

    const today = defaultStartDate ? startDate : activeDateString;
    cy.getCalendarDate(today).click({ force: true });

    cy.get('@calendarStartDate').should(
        'have.value',
        startDate.format('MM/DD/yyyy')
    );
    cy.get('[data-testid=modalTitle]').click();

    cy.get('@calendarEndDate').should('have.value', '').click();

    cy.getCalendarDate(nextPhaseDate).click({ force: true });

    cy.get('@calendarEndDate').should(
        'have.value',
        nextPhaseDate.format('MM/DD/yyyy')
    );
...
function baguetteBakeryProductShouldBeArchived() {
    ...
    cy.contains('[data-testid*="archivedProduct_"]', 'Baguette Bakery').should(
        'exist'
    );

    cy.get('[data-testid="calendarToggle"]').click();
    const newDate = moment(activeDateString)
        .subtract(1, 'days')
