...
describe('People', () => {
    ...
    beforeEach(() => {
        ...
        cy.log(
            '**The archived and unassigned drawers should be closed by default**'
        );
        cy.contains('Adam Sandler').should('not.exist');
        cy.contains(
            '[data-testid*=archivedProduct_]',
            'Archived Product'
        ).should('not.exist');
    });
    ...
    it('Create a new assigned person', () => {
        ...
        cy.getCalendarDate(activeDateString).should(
            'have.class',
            'react-datepicker__day--today'
        );
        cy.getCalendarDate(notActiveDateString)
            .should('not.have.class', highlightClass)
            .click();

        cy.route(
            'GET',
            `${Cypress.env('API_PRODUCTS_PATH')}?requestedDate=${moment(
                notActiveDateString
            ).format('yyyy-MM-DD')}`
        ...
        cy.get('@reassignmentDrawer')
            ...
            .find('[data-testid=reassignmentDrawerCountBadge]')
            .should('not.exist');
        ...
        cy.wait(['@postNewPerson', '@getProductsByDate', '@getPeople'])
            .should((xhrs: Cypress.ObjectLike[]) => {
                ...
                expect(
                    '@getUpdatedProduct status: ' + getUpdatedProductXhr?.status
                ).to.equal('@getUpdatedProduct status: ' + 200);
                expect(
                    '@postNewPerson status: ' + postNewPersonXhr.status
            ...
            .then(() => {
                ...
                ensureNewAssignmentIsPresentInReassignmentDrawer(
                    assignedPerson
                );
    ...
    it('Add an existing person to a product', () => {
        ...
        cy.getModal()
            .contains('[data-testid="assignButton"]', 'Assign')
            .click();
    ...
    it('Create a new unassigned person', () => {
        ...
        cy.wait(['@postNewPerson', '@getProductsByDate', '@getPeople'])
            .should((xhrs: Cypress.ObjectLike[]) => {
                ...
                expect(personData.spaceRole.name).to.equal(
                    unassignedPerson.role
                );
            ...
            .then(() => {
                ...
                ensureUnassignedPersonIsPresentInUnassignedDrawer(
                    unassignedPerson
                );
            });
    });
    ...
    it('Edit a person', () => {
        cy.route('PUT', Cypress.env('API_PERSON_PATH') + '/**').as(
            'updatePerson'
        );
        ...
        cy.wait('@updatePerson').should((xhr: Cypress.ObjectLike) => {
            const personData = xhr.response.body || {};
            expect('@updatedPersonXHR status: ' + xhr?.status).to.equal(
                '@updatedPersonXHR status: ' + 200
            );
    ...
    it('Delete a person', () => {
        cy.route('DELETE', Cypress.env('API_PERSON_PATH') + '/**').as(
            'deletePerson'
        );
        ...
        cy.wait('@deletePerson').should((xhr: Cypress.ObjectLike) => {
            expect(xhr?.status).to.equal(200);
        });
        cy.get('[data-testid=editPersonIconContainer__jane_smith]').should(
            'not.exist'
        );
    });
    ...
    context('Drag and Drop', () => {
        it('Drag and drop person from one product to another product and back', () => {
            ...
            const janeSmithSelector =
                '[data-testid=assignmentCard__jane_smith]';
        ...
        it('Drag and drop person from a product to unassigned and back', () => {
            ...
            const adamSandlerSelector =
                '[data-testid="assignmentCard__adam_sandler"]';
    ...
    context('Person/Assignment Menu', () => {
        ...
        it('Make and unmark a person as a placeholder', () => {
            cy.get('[data-testid=assignmentCard__jane_smith]').should(
                'not.have.class',
                'placeholder'
            );
        ...
        it('Cancel an assignment', () => {
            ...
            cy.get('[data-testid="unassignedDrawer"]').should(
                'not.contain',
                assignmentName
            );
...
const submitPersonForm = (expectedSubmitButtonText: string): void => {
    cy.get('[data-testid=personFormSubmitButton]')
        .should('have.text', expectedSubmitButtonText)
        .click();
    cy.get('@personForm').should('not.exist');
};

const ensureNewAssignmentIsPresentInReassignmentDrawer = (
    assignedPerson: Person
): void => {
    cy.get('@reassignmentDrawer')
        .find(
            '[data-testid=reassignmentContainer] [data-testid=reassignmentSection]'
        )
...
const ensureUnassignedPersonIsPresentInUnassignedDrawer = (
    unassignedPerson: Person
): void => {
    cy.get('[data-testid=unassignedDrawer]').as('unassignedDrawer');
    cy.get('@unassignedDrawer')
        .should('contain', 'Unassigned')
        .find('[data-testid=unassignedDrawerCountBadge]')
        .should('have.text', '2');

    cy.get('@unassignedDrawer')
        .find(
            '[data-testid=unassignedPeopleContainer] [data-testid=assignmentCard__person_name]'
        )
        .should('contain', unassignedPerson.name)
        .should('contain', unassignedPerson.role);
};
...
function checkIfPersonIsInUnassignedDrawer(name: string, isPresent = false) {
    ...
    cy.get('[data-testid=productDrawerContainer__unassigned]').should(
        shouldAssertion,
        name
    );
}

function checkIfPersonIsInProductBaguetteBakery(
    name: string,
    isPresent = false
) {
    const shouldAssertion = isPresent ? 'contain' : 'not.contain';
...
function moveAssignment(
    selector: string,
    direction: typeof keycodes.arrowLeft | typeof keycodes.arrowLeft
) {
