...
describe('ReassignedDrawer', () => {
    ...
    describe('Archived people', () => {
        beforeEach(async () => {
            ...
            renderWithRecoil(
                <MemoryRouter
                    initialEntries={['/bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb']}
                >
                    <Routes>
                        <Route
                            path="/:teamUUID"
                            element={<ReassignedDrawer />}
                        />
                    </Routes>
                </MemoryRouter>,
                ({ set }) => {
            ...
            await waitFor(() =>
                expect(AssignmentClient.getReassignments).toHaveBeenCalled()
            );
        });

        it('should show that they have been archived', async () => {
            expect(
                screen.getByText(TestData.archivedPerson.name)
            ).toBeInTheDocument();
            expect(
                screen.getByText(TestData.archivedPerson.spaceRole!.name)
            ).toBeInTheDocument();
        ...
        it('should unarchive an archived person that gets reverted', async () => {
            AssignmentClient.deleteAssignmentForDate = jest
                .fn()
                .mockResolvedValue({ data: [] });
            PeopleClient.updatePerson = jest.fn().mockResolvedValue({
                data: { ...TestData.archivedPerson, archiveDate: null },
            });
            ...
            await waitFor(() =>
                expect(
                    AssignmentClient.deleteAssignmentForDate
                ).toHaveBeenCalledTimes(1)
            );
            expect(
                AssignmentClient.deleteAssignmentForDate
            ).toHaveBeenCalledWith(mayFourteen2020, TestData.archivedPerson);
