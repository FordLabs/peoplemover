...
const personTagsAlphabetical = [TestData.personTag2, TestData.personTag1];

describe('useFetchPersonTags Hook', () => {
    it('should fetch all person tags for space and store them in recoil alphabetically', async () => {
        PersonTagClient.get = jest
            .fn()
            .mockResolvedValue({ data: personTagsNotAlphabetical });

        const { result } = renderHook(() => useFetchPersonTags(spaceUUID), {
            wrapper,
        });
