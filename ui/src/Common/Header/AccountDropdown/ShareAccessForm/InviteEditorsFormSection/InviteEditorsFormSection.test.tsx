...
describe('Invite Editors Form', function () {
    ...
    beforeEach(() => {
        SpaceClient.getUsersForSpace = jest
            .fn()
            .mockResolvedValue(TestData.spaceMappingsArray);
    ...
    describe('Feature toggle enabled', () => {
        describe('Add editors', () => {
            it('should render the local test space', async () => {
                ...
                expect(
                    screen.queryByText('Enter CDSID of your editors')
                ).not.toBeInTheDocument();
                screen.getByText('hford1');

                fireEvent.click(
                    screen.getByTestId('inviteEditorsFormSubmitButton')
                );

                await waitFor(() =>
                    expect(SpaceClient.inviteUsersToSpace).toHaveBeenCalledWith(
                        space,
                        ['hford1']
                    )
                );
            ...
            it('should not add invalid cdsid user as editor', async function () {
                ...
                const submitButton = screen.getByTestId(
                    'inviteEditorsFormSubmitButton'
                );
                expect(submitButton).toHaveAttribute('disabled');
                expect(
                    screen.queryByTestId('inviteEditorsFormErrorMessage')
                ).toBeInTheDocument();
            ...
            describe('Submit Button and Error Message', () => {
                ...
                it('should be enabled when there are entries, and disabled when there are none', async () => {
                    ...
                    // Delete bford
                    inputField = screen.getByLabelText(
                        /People with this permission can edit/
                    );
        ...
        it('should show owners and editors for the space', async () => {
            ...
            const ownerRow = within(
                await screen.findByTestId('userListItem__user_id')
            );
            ownerRow.getByText(/owner/i);
            ownerRow.getByText(/user_id/i);

            const editorRow = within(
                await screen.findByTestId('userListItem__user_id_2')
            );
        ...
        it('should not open UserAccessList popup', async () => {
            ...
            await waitFor(() => {
                expect(
                    screen.queryByTestId('userAccess')
                ).not.toBeInTheDocument();
            });
        });
    });
...
async function renderComponent(
    currentUser = 'User_id',
    currentSpace: Space = TestData.space
): Promise<void> {
    renderWithRecoil(
