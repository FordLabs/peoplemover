...
describe('ProductCard', () => {
    ...
    const products = [
        ...
        {
            ...TestData.productForHank,
            assignments: [
                TestData.assignmentForHank,
                {
                    ...TestData.assignmentForPerson1,
    ...
    describe('Product Name', () => {
        ...
        it('should render product name as a link if url is present', async () => {
            ...
            expect(
                within(productNameLink).getByTestId('productUrl')
            ).toBeDefined();
        });
    });
    ...
    describe('Read only view', () => {
        ...
        it('should not show edit product icon', async () => {
            expect(
                screen.queryByTestId(/editProductIcon/i)
            ).not.toBeInTheDocument();
        });

        it('should not show add assignment icon', async () => {
            expect(
                screen.queryByTestId(/addPersonToProductIcon/i)
            ).not.toBeInTheDocument();
        });

        it('should hide the add person icon when read only', () => {
            expect(
                screen.queryByTestId('addPersonToProductIcon__unassigned')
            ).not.toBeInTheDocument();
        });
    });
    ...
    it('should not display message when product has assignments', async () => {
        renderProductCard(TestData.productWithoutAssignments);
        expect(
            screen.queryByText('Add a person by clicking')
        ).not.toBeInTheDocument();
    });
    ...
    it('should order the AssignmentCards on the product by role, then name, then id', async () => {
        ...
        const assignmentCardIds = productWithManyAssignments.assignments.map(
            (assignment) => assignment.id
        );
        expect(assignmentCardIds.length).toBe(4);

        expectedPersonsInOrder.forEach((person) => {
            const assignmentContainerDiv = screen.getByTestId(
                createDataTestId('assignmentCard', person.name)
            );
    ...
    it('archiving a product sets the appropriate fields in the product and moves all people to unassigned', async () => {
        const may13String = moment(mayFourteenth2020)
            .subtract(1, 'day')
            .format('YYYY-MM-DD');
        ...
        const testProduct = {
            ...
            assignments: [
                TestData.assignmentForPerson1,
                TestData.assignmentForPerson2,
                TestData.assignmentForPerson3,
            ],
        };

        ProductClient.editProduct = jest.fn().mockResolvedValue({
            data: { ...testProduct, endDate: may13String },
        });
        ...
        expect(AssignmentClient.createAssignmentForDate).toHaveBeenCalledWith(
            may14String,
            [{ productId: TestData.productForHank.id, placeholder: false }],
            TestData.space,
            TestData.person1
        );
        expect(AssignmentClient.createAssignmentForDate).toHaveBeenCalledWith(
        ...
        expect(ProductClient.editProduct).toHaveBeenCalledWith(TestData.space, {
            ...testProduct,
            endDate: may13String,
        });
    });
    ...
    it('should show a confirmation modal when Archive Person is clicked, and be able to close it', async () => {
        ...
        const editProductSelector = createDataTestId(
            'editProductIcon',
            TestData.productWithAssignments.name
        );
    ...
    function renderProductCard(product: Product, isReadOnly = false) {
        renderWithRecoil(<ProductCard product={product} />, ({ set }) => {
