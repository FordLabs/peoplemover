...
describe('Assignment Form', () => {
    ...
    it('renders the assignment form labels', () => {
        renderWithRecoil(
            <AssignmentForm
                initiallySelectedProduct={TestData.unassignedProduct}
            />,
            ({ set }) => {
    ...
    it('accepts changes to the assignment forms product list and can submit multiple assignments', async () => {
        ...
        const containerToFindOptionsIn = {
            container: await screen.findByTestId('assignmentForm'),
        };
    ...
    describe('In create mode', () => {
        it('should not show the unassigned or archived products in the product list', async () => {
            ...
            expect(
                within(productsMultiSelectField).queryByText('I am archived')
            ).toBeNull();
            expect(
                within(productsMultiSelectField).queryByText('unassigned')
            ).toBeNull();
        });
        ...
        it('should populate dropdown to create new person with whatever is typed in input field', async () => {
            renderComponent();
            const labelElement = await screen.findByLabelText('Name');
            fireEvent.change(labelElement, {
                target: { value: 'Barbara Jordan' },
            });
            await screen.findByText(
                TestUtils.expectedCreateOptionText('Barbara Jordan')
            );
        });
        ...
        it('populates the person name field of the Create Person modal on open', async () => {
            ...
            const createOptionText =
                TestUtils.expectedCreateOptionText('XYZ ABC 123');
            fireEvent.click(
                getByText(
                    await screen.findByTestId('assignmentForm'),
                    createOptionText
                )
            );
...
const prefillReactSelectField = async (
    label: string,
    prefillText: string
): Promise<void> => {
