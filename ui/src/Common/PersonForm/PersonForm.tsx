...
function PersonForm({
    ...
    const setIsUnassignedDrawerOpen = useSetRecoilState(
    ...
    const [isPersonNameInvalid, setIsPersonNameInvalid] =
    ...
    const [hasAssignmentChanged, setHasAssignmentChanged] =
        useState<boolean>(false);
    const [hasNewPersonChanged, setHasNewPersonChanged] =
        useState<boolean>(false);
    const [initialNewPersonFlag, setInitialNewPersonFlag] =
        useState<boolean>(false);
    ...
    const getUnassignedProductId = useCallback((): number => {
        const unassignedProduct: Product | undefined = products.find(
            (product: Product) => product.name === 'unassigned'
        );
        if (unassignedProduct && unassignedProduct.id)
            return unassignedProduct.id;
        return -1;
    }, [products]);

    const createProductsFromAssignments = useCallback(
        (assignments: Assignment[]): Product[] => {
            const allProductIdsFromAssignments = assignments.map(
                (a) => a.productId
            );
            return products
                .filter((p) => allProductIdsFromAssignments.includes(p.id))
                .filter((product) => product.id !== getUnassignedProductId());
        },
        [getUnassignedProductId, products]
    );

    const populatedEntirePersonForm = useCallback(
        (personToPopulate: Person): void => {
    ...
    useEffect(() => {
        if (isEditPersonForm && personEdited) {
            ...
            if (initialPersonName) {
                setPerson((updatingPerson: Person) => ({
                    ...updatingPerson,
    ...
    const getSelectedProductPairs = (): ProductPlaceholderPair[] => {
        return selectedProducts.map((product) => {
            const placeholderForProduct = product.assignments.find(
                (assignmentForProduct) =>
                    assignmentForProduct.person.id === person.id
            )?.placeholder;
            return {
    ...
    const handleSubmit = async (event: FormEvent): Promise<void> => {
        ...
        if (person.name.trim() === '') {
            ...
            if (isEditPersonForm) {
                const response = await PeopleClient.updatePerson(
                    currentSpace,
                    personToSend
                );
                ...
                const response = await PeopleClient.createPersonForSpace(
                    currentSpace,
                    personToSend
                );
    ...
    const changeProductName = (events: Array<{ value: string }>): void => {
        ...
        setSelectedProducts(
            updatedProducts.filter((product) => product != null)
        );
    ...
    const updatePersonField = (
        fieldName: string,
        fieldValue: string | boolean | RoleTag | Date | undefined
    ): void => {
        setPerson((updatingPerson: Person) => ({
            ...updatingPerson,
    ...
    const updateSpaceRole = (input: string): void => {
        const roleMatch: RoleTag | undefined = roles.find(
            (role: RoleTag) => role.name === input
        );
    ...
    const handleCreateRole = (inputValue: string): void => {
        ...
        RoleClient.add(roleAddRequest, currentSpace).then(
    ...
    const displayRemovePersonModal = (): void => {
        const propsForDeleteConfirmationModal: ConfirmationModalProps = {
            ...
            content: (
                <div>
                    Removing this person will remove all instances of them from
                    your entire space.
                </div>
            ),
        };
    ...
    const getAssignToOptions = (): Array<Option> => {
        const filteredProducts: Array<Product> = products.filter(
            (product) =>
                isActiveProduct(product, viewingDate) &&
                !isUnassignedProduct(product)
        );
        alphabetize(filteredProducts);
        return filteredProducts.map((selectable) => {
    ...
    const toolTipContent = (): JSX.Element => {
        return (
            <span className="toolTipContent">
                Create tags based on your people. Example, skills, education,
                employee status, etc. Anything on which you would like to
                filter.
            </span>
        );
    };
        ...
        <div className="formContainer">
            ...
            >
                <div className="formItem">
                    ...
                    />
                    {isPersonNameInvalid && (
                        <span className="personNameWarning">
                    ...
                    <div className="isNewContainer">
                        ...
                        />
                        <label className="formInputLabel" htmlFor="isNew">
                            Mark as New
                        </label>
                    </div>
                </div>
                <div className="formItem">
                    <label className="formItemLabel" htmlFor="cdsid">
                        CDSID
                    </label>
                    <input
                        className="formInput formTextInput"
                ...
                <SelectWithCreateOption
                    ...
                    useColorBadge
                    value={
                        person.spaceRole && person.spaceRole.name !== ''
                            ? createOption(person.spaceRole)
                            : undefined
                    }
                    options={roles.map((role) => createOption(role))}
                    onChange={(e): void =>
                        updateSpaceRole(e ? (e as Option).value : '')
                    }
                ...
                <FormTagsField
                    ...
                    selectedTagsState={{
                        selectedTags: selectedPersonTags,
                        setSelectedTags: setSelectedPersonTags,
                    }}
