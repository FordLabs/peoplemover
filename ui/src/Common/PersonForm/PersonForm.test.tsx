...
describe('Person Form', () => {
    ...
    beforeEach(() => {
        AssignmentClient.getAssignmentsUsingPersonIdAndDate = jest
            .fn()
            .mockResolvedValue({
                data: [{ ...TestData.assignmentForPerson1 }],
            });
    });

    it('should have correct placeholder texts and defaults', async () => {
        renderWithRecoil(<PersonForm isEditPersonForm={false} />, ({ set }) => {
            set(CurrentSpaceState, TestData.space);
        });

        await waitFor(() =>
            expect(screen.getByPlaceholderText('e.g. Jane Smith')).toBeDefined()
        );
    ...
    it('should pre-populate form', async () => {
        ...
        await waitFor(() =>
            ...
            ).toHaveBeenCalledWith(TestData.space.uuid, person.id, viewingDate)
        );
        ...
        expect(
            screen.getByText(`${expectedNote.length} (255 characters max)`)
        ).toBeDefined();
    });
    ...
    it('should not show the unassigned product or archived products in product list', async () => {
        ...
        renderWithRecoil(
            <PersonForm isEditPersonForm={false} initialPersonName="BRADLEY" />,
            ({ set }) => {
        ...
        const productsMultiSelectField = await screen.findByLabelText(
            'Assign to'
        );
    ...
    describe('Creating a new person', () => {
        ...
        it('create new person tags when one is typed in which does not already exist', async () => {
            ...
            const expectedPersonTagAddRequest: TagRequest = {
                name: 'Low Achiever',
            };
            await expect(PersonTagClient.add).toHaveBeenCalledWith(
                expectedPersonTagAddRequest,
                TestData.space
            );
        ...
        it('should update newPersonDate on person when newPerson field goes from unchecked to checked for edit person', async () => {
            ...
            await waitFor(() =>
                expect(PeopleClient.createPersonForSpace).toHaveBeenCalledWith(
                    TestData.space,
                    expectedPerson
                )
            );
        ...
        it('should populate person form and create person', async () => {
            fireEvent.change(screen.getByLabelText('Name'), {
                target: { value: 'New Bobby' },
            });
            fireEvent.change(screen.getByLabelText('Role'), {
                target: { value: 'Software Engineer' },
            });
            fireEvent.change(screen.getByLabelText('CDSID'), {
                target: { value: 'btables1' },
            });
            fireEvent.click(screen.getByLabelText('Mark as New'));

            await selectEvent.create(
                await screen.findByLabelText('Person Tags'),
                'Low Achiever'
            );
    ...
    describe('Editing an existing person', () => {
        const setupPersonForm = async () => {
            AssignmentClient.getAssignmentsV2ForSpaceAndPerson = jest
                .fn()
                .mockResolvedValue({
                    data: [
                        { ...TestData.assignmentForHank, endDate: null },
        ...
        it('should only display active assignable projects in the assignment dropdown', async () => {
            ...
            expect(
                screen.queryByText(TestData.unassignedProduct.name)
            ...
            expect(
                screen.queryByText(TestData.archivedProduct.name)
            ).not.toBeInTheDocument();
            await screen.findByText(TestData.productWithoutLocation.name);
            expect(
                screen.queryByText('Already Closed Product')
            ).not.toBeInTheDocument();
        });
    ...
    describe('handleSubmit()', () => {
        ...
        it('should call createAssignmentForDate when assignment has been deliberately changed to unassigned', async () => {
            PeopleClient.updatePerson = jest
                .fn()
                .mockResolvedValue({ data: TestData.hank });
            ...
            const removeProductButton = container.getElementsByClassName(
                'product__multi-value__remove'
            );
        ...
        it('should update newPersonDate on person when newPerson field goes from unchecked to checked for edit person', async () => {
            ...
            fireEvent.click(
                await screen.findByTestId('personFormIsNewCheckbox')
            );
            fireEvent.click(await screen.findByText('Save'));
            ...
            await waitFor(() =>
                expect(PeopleClient.updatePerson).toHaveBeenCalledWith(
                    TestData.space,
                    expectedPerson
                )
            );
        ...
        it('should send a regular assignment request on an unarchived person', async () => {
            const updatedPerson = {
                ...TestData.archivedPerson,
                archiveDate: null,
            };
            PeopleClient.updatePerson = jest
                .fn()
                .mockResolvedValue({ data: updatedPerson });
            ...
            AssignmentClient.getAssignmentsUsingPersonIdAndDate = jest
                .fn()
                .mockResolvedValue({ data: [TestData.assignmentForArchived] });
            ...
            await selectEvent.openMenu(
                await screen.findByLabelText('Assign to')
            );
            await screen.findByText(TestData.productWithoutAssignments.name);
    ...
    describe('Deleting a person', () => {
        beforeEach(async () => {
            renderWithRecoil(
                <PersonForm isEditPersonForm={true} />,
                recoilState
            );
            await waitFor(() => expect(PersonTagClient.get).toHaveBeenCalled());
