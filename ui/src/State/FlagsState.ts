...
export const simplifyFlags = (flags?: IFlags | null): Flag => {
    if (flags) {
        const announcementBannerEnabled =
            flags['announcement_banner_enabled'].enabled;
        const announcementBannerMessage =
            flags['announcement_banner_message']?.value || '';
