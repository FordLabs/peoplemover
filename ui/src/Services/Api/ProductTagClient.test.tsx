...
describe('Product Tags Client', function () {
    ...
    beforeEach(() => {
        ...
        Axios.post = jest
            .fn()
            .mockResolvedValue({ data: 'Created Product Tag' });
        Axios.put = jest
            .fn()
            .mockResolvedValue({ data: 'Updated Product Tag' });
        Axios.delete = jest
            .fn()
            .mockResolvedValue({ data: 'Deleted Product Tag' });
    ...
    it('should return all product tags for space', function (done) {
        ProductTag.get(spaceUuid).then((response) => {
    ...
    it('should create a product tag and return that product tag', function (done) {
        ...
        ProductTag.add(expectedProductAddRequest, TestData.space).then(
            (response) => {
                expect(Axios.post).toHaveBeenCalledWith(
                    baseProductTagsUrl,
                    expectedProductAddRequest,
                    expectedConfig
                );
    ...
    it('should edit a product tag and return that product tag', function (done) {
        ...
        ProductTag.edit(expectedProductEditRequest, TestData.space).then(
            (response) => {
                expect(Axios.put).toHaveBeenCalledWith(
                    `${baseProductTagsUrl}/${TestData.productTag1.id}`,
                    expectedProductEditRequest,
                    expectedConfig
                );
    ...
    it('should delete a product tag', function (done) {
        ...
        ProductTag.delete(TestData.productTag1.id, TestData.space).then(
            (response) => {
