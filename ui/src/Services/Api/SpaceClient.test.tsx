...
describe('Space Client', function () {
    ...
    it('should get all the users for a space with the owner first', function (done) {
        ...
        Axios.get = jest.fn().mockResolvedValue({
            data: [
                { userId: 'user_id_2', permission: 'editor' },
                { userId: 'user_id', permission: 'owner' },
            ],
        });
    ...
    it('should remove users from space', (done) => {
        const user: UserSpaceMapping = {
            id: 'blah',
            userId: 'user1',
            spaceUuid: `${TestData.space.uuid}`,
            permission: 'fakePermission',
        };
        SpaceClient.removeUser(TestData.space, user).then(() => {
    ...
    it('should assign owner permission to specified user from space', (done) => {
        const newOwner: UserSpaceMapping = {
            id: 'blah',
            userId: 'newOwner',
            spaceUuid: `${TestData.space.uuid}`,
            permission: 'editor',
        };
        const currentOwner: UserSpaceMapping = {
            id: 'blah',
            userId: 'currentOwner',
            spaceUuid: `${TestData.space.uuid}`,
            permission: 'owner',
        };
        SpaceClient.changeOwner(TestData.space, currentOwner, newOwner).then(
            () => {
                expect(Axios.put).toHaveBeenCalledWith(
