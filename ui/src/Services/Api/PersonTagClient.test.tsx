...
describe('Person Tags Client', function () {
    ...
    beforeEach(() => {
        ...
        Axios.post = jest
            .fn()
            .mockResolvedValue({ data: 'Created Person Tag' });
        Axios.put = jest.fn().mockResolvedValue({ data: 'Updated Person Tag' });
        Axios.delete = jest
            .fn()
            .mockResolvedValue({ data: 'Deleted Person Tag' });
    ...
    it('should return all person tags for space', function (done) {
        PersonTag.get(spaceUuid).then((response) => {
    ...
    it('should create a person tag, return that person tag, and send a `person tag created` event to Matomo', function (done) {
        ...
        PersonTag.add(expectedPersonAddRequest, TestData.space).then(
            (response) => {
                expect(Axios.post).toHaveBeenCalledWith(
                    basePersonTagsUrl,
                    expectedPersonAddRequest,
                    expectedConfig
                );
    ...
    it('should edit a person tag and return that person tag', function (done) {
        ...
        PersonTag.edit(expectedPersonEditRequest, TestData.space).then(
            (response) => {
                expect(Axios.put).toHaveBeenCalledWith(
                    `${basePersonTagsUrl}/${TestData.personTag1.id}`,
                    expectedPersonEditRequest,
                    expectedConfig
                );
    ...
    it('should delete a person tag', function (done) {
        ...
        PersonTag.delete(TestData.personTag1.id, TestData.space).then(
            (response) => {
