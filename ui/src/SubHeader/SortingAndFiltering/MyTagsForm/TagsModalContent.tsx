...
const TagsModalContent = ({
    ...
    const [editTagIndex, setEditTagIndex] = useState<number>(
    ...
    const showDeleteConfirmationModal = (tagToDelete: Tag): void => {
        const propsForDeleteConfirmationModal: ConfirmationModalProps = {
            ...
            content: (
                <div>
                    Deleting this {filterType.tagType} will remove it from
                    anything that has been given this {filterType.tagType}.
                </div>
            ),
        };
    ...
    const editTag = async (tagToEdit: TagRequest): Promise<unknown> => {
        return await tagClient.edit(tagToEdit, currentSpace).then(() => {
            fetchCommand();
            returnToViewState();
        });
    };
    ...
    const addTag = async (tagToAdd: TagRequest): Promise<unknown> => {
        return await tagClient.add(tagToAdd, currentSpace).then(() => {
            fetchCommand();
            returnToViewState();
        });
    };
    ...
    const showEditButtons = (): boolean =>
        editTagIndex === INACTIVE_EDIT_STATE_INDEX && !isAddingNewTag;
                        ...
                        {showViewState(index) && (
                            <ViewTagRow
                                ...
                                setConfirmDeleteModal={(): void =>
                                    showDeleteConfirmationModal(currentTag)
                                }
